#' @title shiftCor
#' @description  shiftCor provide the QC-RLS correction for large scale metabolomics.
#' @param samPeno The file with  the meta information including the sample name, batches, class and order. 
...
shiftCor = function(samPeno,samFile,Frule = 0.8,QCspan = 0.75, degree = 2,imputeM = "KNN"){
  ...
  if(dim(samFile)[1] - dim(samPeno)[1]>0){
    message("\nWarning: The sample size in Profile is larger than Pheno! ")
  }else if(dim(samFile)[1] - dim(samPeno)[1]<0) {
    stop("\nThe sample size in Profile is smaller than Pheno! Check your data please!!")
  }
  ...
  if(sum(is.na(sam_seq_tmp$class)) > 0 ){
    stop("\nThere were NA values (ungrouped data) in sample class! Check your data please!\n")
  }
  ...
  if(imputeM == "KNN"){
    #require(impute)
    mvd <- impute::impute.knn(imsamFP[,2:ncol(imsamFP)],rowmax = 0.99, colmax = 0.99, maxp = 15000)
  ...
  if(QCspan > 0){
    ...
    loessFit=function(x,y,QCspan,degree){
      ...
      if(length(ed_QC)==0)
      {
        stop("\nWrong: the sample at the end of sequence must be QC sample; please check ......");
      }
      qcid <- grep("QC",cn)
...
###############
  ...
  {  
    ...
    mvd2 <- impute::impute.knn(loessDat[,1:ncol(loessDat)], rowmax = 0.99, colmax = 0.99, maxp = 15000)
    loessDat <- mvd2$data
  ...
  #Rsdist_QC_raw = RsdCal(paste(dirout.Bs, RSD30_CV, sep="/"),batch = TRUE, DistPattern = TRUE, output = FALSE)
  ...
  #Rsdist_sam_raw = RsdCal(paste(dirout.Bs, RSD30_CV, sep="/"),batch = FALSE, DistPattern = FALSE,output = FALSE)
  ...
  ############loess sample Cal#############
  #Rsdist_sam_cor = RsdCal(paste(dirout.As, RSD30_CV, sep="/"),batch = FALSE, DistPattern = FALSE,output = FALSE)
  ...
  #Rsdist_QC_cor=RsdCal(paste(dirout.As, RSD30_CV, sep="/"),batch = TRUE, DistPattern = TRUE,output = TRUE)
