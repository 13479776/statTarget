...
#' @description welch This function provide two.sided Student's T-tests for each pairwise comparison of sample groups.
#' @param file The connection to the data file.
#' @return A matrix with p value 
welch <-
function(file){
 ...
 for (i in 1:NoF) {
  for (j in 1:NoF) { 
   if (i < j) {
    ...
    welch.ij=paste("WelchTest_",ExcName(i,slink),"vs",ExcName(j,slink), ".csv", sep="")
    assign(welch.ij,we)
