...
function(pcx,pcy,scaling,Labels) {
 ...
 pdf(D)
 #dev.new()
 graphics::plot(Score.x[,pcx], Score.x[,pcy], col=col, xlab = xlab, ylab = ylab, xlim = lim, ylim = lim, pch=19, sub = paste("Cumulative Proportion of Variance Explained = ", cum, "%", sep=""), main = paste("PCA Score Plot (", scaling, ")", sep=""))
 axis(1, at=lim*2, pos=c(0,0), labels=FALSE, col="grey", lwd=0.7)
 axis(2, at=lim*2, pos=c(0,0), labels=FALSE, col="grey", lwd=0.7)
 #requireNamespace(car)
 #library(car)
 dataEllipse_sT(Score.x[,pcx], Score.x[,pcy], levels = c(0.95), add=TRUE, col = "grey48", lwd = 0.4, plot.points=FALSE, center.cex=0.2)
