#' @title Volcano Plot
#' @description  volcano provide the differential markers for metabolomics analysis.
#' @param file The connection for P-values and Fold changes of metabolites.
...
for (i in 1:NoF) {
  for (j in 1:NoF) {
   if (i < j) {
    ...
    pv = paste(getwd(), "/Univariate/Pvalues/Pvalues_", ExcName(i,slink), "vs", ExcName(j,slink), ".csv", sep="")
...
fc.csvfile = paste("Fold_Change_", ExcName(i,slink), "vs",ExcName(j,slink), ".csv", sep="")
...
V = paste(dirout.vol, "VolcanoPlot_", ExcName(i,slink), "vs", ExcName(j,slink), ".pdf", sep="")
...
pdf(V)
graphics::plot(logfc, logpv, col=colpv, cex= 1.2,pch = 19, xlim=c(-max.fc,max.fc), xlab = "Log2 (Fold Change)", ylab = "Log10 (Pvalue)", main = paste("Volcano Plot ", ExcName(i,slink), " vs ", ExcName(j,slink), sep=""), sub = paste("(Variables in Blue are significant (Pvalue<",sig.lim, ") and showed Fold Changes>",upper.lim," or <",lower.lim,")", sep = ""))
if(length(colnames(sorted.x)[grep("navy",colpv)]) > 0){
text(logfc[grep("navy",colpv)], logpv[grep("navy",colpv)], labels=colnames(sorted.x)[grep("navy",colpv)], cex=0.6, pos=pospv, col=colpv[grep("navy",colpv)])
   } else {
 }
